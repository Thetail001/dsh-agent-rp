...
function assertPublicDiscussionStatement(
  ...
  if (publicStatementMisusesNightOutcomeCorroboration(statement)) {
    throw new DecisionValidationError(
      'night-outcome-corroboration',
      `${options.label} treated a player's night survival or death as evidence for a Seer result`,
    )
  }
...
async function coordinateDiscussion(
  ...
  for (const [index, actorId] of actors.entries()) {
    ...
    const task = `进行第 ${String(round)} 天公开发言。你是${seatLabel(actorId)}。${positionInstruction}`
      ...
      + '同一目标的同类关注只保留首位玩家提出的判断，后位最多再由一人承接收口；已有判断且没有目标本人的新发言、后来的公开票型或阶段事实时直接 pass。'
      ...
      + '预言家验谁与狼人刀谁是两项独立行动；查验目标存活、死亡或另一名玩家夜间死亡都不能印证或反驳查验。'
