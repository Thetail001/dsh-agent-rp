...
import type { TavernInjectedPrompt, TavernWorldbookBindings, TavernWorldbookEntry } from '../tavern-helper.ts'
...
export interface TavernScriptSnapshot {
  ...
  readonly presetScriptTrees: readonly JsonRecord[]
  /** Current character-card scripts in Tavern Helper's public tree representation. */
  readonly characterScriptTrees: readonly JsonRecord[]
...
var __dshInjectionWrite=Promise.resolve(),__dshInjectionReady=Promise.resolve();
...
function __dshTrackInjectionFilter(task){__dshInjectionReady=Promise.all([__dshInjectionReady,task]).then(function(){})}
...
function __dshStoreInjectedPrompt(prompt,persist){__dshInjectedPrompts=__dshInjectedPrompts.filter(function(value){return value.id!==prompt.id}).concat([prompt]);if(persist!==false)__dshPersistInjections()}
window.injectPrompts=function(prompts,option){if(!Array.isArray(prompts)||prompts.length>256)throw new Error('注入提示词列表无效');if(option?.once!==undefined&&typeof option.once!=='boolean')throw new Error('注入提示词 once 无效');var prepared=prompts.map(function(source,index){return {source:source,prompt:__dshInjectedPrompt(source,index,option?.once===true)}});if(prepared.reduce(function(total,item){return total+item.prompt.content.length},0)>1048576)throw new Error('注入提示词合计过长');var active=true,changed=false,ids=prepared.map(function(item){return item.prompt.id});for(let item of prepared){let source=item.source,prompt=item.prompt;if(typeof source.filter!=='function'){__dshStoreInjectedPrompt(prompt,false);changed=true;continue}try{var filtered=source.filter();if(filtered&&typeof filtered.then==='function'){var task=Promise.resolve(filtered).then(function(value){if(active&&value!==false)__dshStoreInjectedPrompt(prompt)}).catch(function(error){__dshPost('runtime-error',{value:String(error)})});__dshTrackInjectionFilter(task)}else if(active&&filtered!==false){__dshStoreInjectedPrompt(prompt,false);changed=true}}catch(error){__dshPost('runtime-error',{value:String(error)})}}if(changed)__dshPersistInjections();return {uninject:function(){active=false;window.uninjectPrompts(ids)}}};
window.uninjectPrompts=function(ids){var targets=new Set((Array.isArray(ids)?ids:[]).map(String));if(targets.size===0)return;var next=__dshInjectedPrompts.filter(function(prompt){return !targets.has(prompt.id)});if(next.length===__dshInjectedPrompts.length)return;__dshInjectedPrompts=next;__dshPersistInjections()};
function __dshConsumeOnceInjections(){if(!__dshInjectedPrompts.some(function(prompt){return prompt.once===true}))return;__dshInjectedPrompts=__dshInjectedPrompts.filter(function(prompt){return prompt.once!==true});__dshPersistInjections()}
...
window.getScriptTrees=function(option){if(!__dshPlain(option)||!['global','preset','character'].includes(option.type))throw new Error("脚本类型必须是 'global'、'preset' 或 'character'");return __dshClone(option.type==='preset'?__dshPresetScriptTrees:option.type==='character'?__dshCharacterScriptTrees:[])};
window.getAllEnabledScriptButtons=function(){var result={};for(var tree of __dshPresetScriptTrees.concat(__dshCharacterScriptTrees)){if(tree?.type!=='script'||tree.enabled===false||tree.button?.enabled!==true)continue;var buttons=(Array.isArray(tree.button.buttons)?tree.button.buttons:[]).filter(function(button){return button?.visible!==false}).map(function(button){return {button_id:String(tree.id)+'_'+String(button.name),button_name:String(button.name)}});if(buttons.length>0)result[String(tree.id)]=buttons}return __dshClone(result)};
...
window.SillyTavern={chat:[],name1:__dshSnapshot.userName??'用户',name2:__dshSnapshot.characterName,characterId:__dshSnapshot.characterId,chatId:__dshSnapshot.chatId,chatMetadata:__dshCurrentChatMetadata,chat_metadata:__dshCurrentChatMetadata,extensionSettings:{},getCurrentCharacterId:window.getCurrentCharId,getCurrentChatId:window.getCurrentChatId,substituteParams:window.substituteParams,eventSource:{on:window.eventOn,once:window.eventOnce,emit:window.eventEmit,emitAndWait:window.eventEmitAndWait,removeListener:window.eventRemoveListener},eventTypes:window.tavern_events,getContext:function(){return this}};
