...
function __dshGenerate(mode,config){var value=__dshClone(config??{});value.__dsh_generation_mode=mode;void __dshEmitLocal(window.iframe_events.GENERATION_STARTED,[]);return window.fetch('/api/backends/chat-completions/generate',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(__dshGenerationBody(value))}).then(async function(response){var raw=await response.text();var result;try{result=JSON.parse(raw)}catch(error){throw new Error(response.ok?'模型返回了无法识别的结果':'生成失败（'+response.status+'）')}if(!response.ok)throw new Error(__dshGenerationError(result));var text=__dshGenerationText(result);if(value.should_stream===true){void __dshEmitLocal(window.iframe_events.STREAM_TOKEN_RECEIVED_FULLY,[text]);void __dshEmitLocal(window.iframe_events.STREAM_TOKEN_RECEIVED_INCREMENTALLY,[text])}void __dshEmitLocal(window.iframe_events.GENERATION_ENDED,[text]);return text})}
