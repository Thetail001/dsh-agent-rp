...
var __dshCardChat=[{message_id:0,message:__dshCardGreetingChoices?.selected??'',mes:__dshCardGreetingChoices?.selected??'',name:__dshCurrentCharacter?.name??'角色',is_user:false,role:'assistant',extra:{},swipe_id:Math.max(0,__dshCardGreetingChoices?.alternatives?.indexOf(__dshCardGreetingChoices.selected)??0),swipes:__dshCardGreetingChoices?.alternatives??[]}];
...
(()=>{'use strict';
  ...
  addEventListener('message',event=>{
    ...
    let activated=navigator.userActivation?.isActive===true;
