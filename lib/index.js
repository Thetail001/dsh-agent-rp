...
function parseCharacterCardJson(json) {
	...
	const { version, specVersion, data } = cardVersion(object(raw, "character card"));
...
function parseResult(value) {
	...
	if (record.version !== 0 || typeof record.name !== "string" || record.cardVersion !== 1 && record.cardVersion !== 2 && record.cardVersion !== 3 || typeof record.sourceEventSeq !== "number" || !Number.isSafeInteger(record.sourceEventSeq) || typeof record.sourceAttachmentId !== "string" || record.metadataKeyword !== "ccv3" && record.metadataKeyword !== "chara" || typeof record.greetingIndex !== "number" || !Number.isSafeInteger(record.greetingIndex) || record.greetingIndex < 0 || typeof record.selectedGreeting !== "string" || !Array.isArray(record.degradations) || record.degradations.some((value) => typeof value !== "string" || !CHARACTER_IMPORT_DEGRADATIONS.includes(value))) throw new Error("import_character_card result has invalid fields");
	return record;
}
...
function sourceImage(events, sourceEventSeq, attachmentId) {
	...
	if (images.length > 0) {
		...
		if (image === void 0) throw new Error("import_character_card source attachment is absent from its user message");
	...
	const image = (sourceMeta.attachmentConsumer === "dsh-agent-rp" && Array.isArray(sourceMeta.attachments) ? sourceMeta.attachments : []).find((value) => typeof value === "object" && value !== null && !Array.isArray(value) && String(value.attachmentId) === attachmentId);
	if (image === void 0) throw new Error("import_character_card source attachment metadata is invalid");
	return image;
}
function validateImport(events, resultEvent) {
	...
	if ((callArguments.greetingIndex ?? 0) !== result.greetingIndex) throw new Error("import_character_card greeting does not match its source call");
	if (result.sourceEventSeq >= call.seq) throw new Error("import_character_card source image does not precede its tool call");
	if (sourceImage(events, result.sourceEventSeq, result.sourceAttachmentId).mediaType !== "image/png") throw new Error("import_character_card source must remain a PNG");
...
*/
function prepareCharacterImportResult(card, payload, sourceEventSeq, image, greetingIndex) {
...
function activateLorebook(book, messages) {
	const depth = book.scanDepth === void 0 ? messages.length : Math.max(0, Math.trunc(book.scanDepth));
...
function substituteCardMacros(value, card) {
...
* @returns model-visible system prompt text.
*/
function renderImportedCharacterPrompt(card, loreBefore, loreAfter) {
	const name = card.nickname?.trim() || card.name;
	const original = `你是${name}。直接以${name}的身份与用户相处和交谈。`;
	const parts = [
		card.systemPrompt.trim().length === 0 ? original : substituteCardMacros(card.systemPrompt, card).replaceAll("{{original}}", original),
	...
	if (card.postHistoryInstructions.trim().length > 0) parts.push(substituteCardMacros(card.postHistoryInstructions, card).replaceAll("{{original}}", ""));
	return parts.join("\n\n");
}
...
function latestUserImages(agent) {
	for (let index = agent.session.events.length - 1; index >= 0; index -= 1) {
		...
		const consumed = source.attachmentConsumer === "dsh-agent-rp" && Array.isArray(source.attachments) ? source.attachments : [];
...
function installAgentRp(ctx, config) {
	...
	ctx.effect(() => gateway.registerPromptAttachmentConsumer("dsh-agent-rp", ({ agent, content }) => {
		if (agentsByScope.get(agent) !== agent) return void 0;
		const images = content.filter((part) => part.type === "image");
		const text = content.filter((part) => part.type === "text").map((part) => part.text).join("\n");
		if (images.length !== 1 || images[0]?.mediaType !== "image/png" || !/(?:角色卡|character\s*card|导入|接管|切换角色)/iu.test(text)) return;
		return { text };
	...
	ctx.systemPrompt.section({
		...
		text: ({ scope }) => {
			...
			if (agent === void 0 || active === void 0) return renderCharacterPrompt(config);
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (!Number.isSafeInteger(imageIndex) || imageIndex < 0 || imageIndex >= images.length) throw new Error(`imageIndex ${imageIndex} is unavailable; the current import source contains ${images.length} image(s)`);
			...
			return prepareCharacterImportResult(parseCharacterCardJson(payload.json), payload, direct.eventSeq, stored.ref, args.greetingIndex ?? 0);
		},
		presentCall: () => ({
			card: "generic",
			title: "导入角色卡",
...
export { CHARACTER_IMPORT_VALUE_SCHEMA, Config, MEMORY_VALUE_SCHEMA, apply, inject, installAgentRp, name };
