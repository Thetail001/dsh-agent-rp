...
function parseRegexScript(value, path) {
	const script = object$6(value, path);
	return {
...
function regex(value) {
	...
	return value.map((item, itemIndex) => {
		const record = object$1(item, `regex[${itemIndex}]`);
...
function content(value) {
	...
	const seen = /* @__PURE__ */ new Set();
	return value.map((item, itemIndex) => {
		const record = object$1(item, `content[${itemIndex}]`);
...
function promptDefinitions(value) {
	...
	const seen = /* @__PURE__ */ new Set();
	return value.map((item, itemIndex) => {
		const record = object$1(item, `prompts[${itemIndex}]`);
...
function presetProjection(name, preset, revision, importedPreset = preset) {
	...
	return {
		...
		regexScriptCount: preset.extensionSummary.regexScriptCount,
		regexScripts: presetRegexScripts(preset).map((script, index) => ({
			...script,
			index
		})),
		appliedGeneration,
