...
function nullableFiniteNumber(value, path) {
	...
	return optionalFiniteNumber$1(value, path) ?? null;
}
function parseFrontend(data) {
	...
	const regexScripts = rawRegex === void 0 ? [] : (() => {
		...
		return rawRegex.map((value, index) => {
			...
			const script = object$5(value, path);
			return {
				...
				trimStrings: stringArray$2(script.trimStrings, `${path}.trimStrings`),
				placement: numberArray(script.placement, `${path}.placement`),
				disabled: optionalBoolean$1(script.disabled, `${path}.disabled`) ?? false,
				markdownOnly: optionalBoolean$1(script.markdownOnly, `${path}.markdownOnly`) ?? false,
				promptOnly: optionalBoolean$1(script.promptOnly, `${path}.promptOnly`) ?? false,
				runOnEdit: optionalBoolean$1(script.runOnEdit, `${path}.runOnEdit`) ?? false,
				substituteRegex: optionalFiniteNumber$1(script.substituteRegex, `${path}.substituteRegex`) ?? 0,
	...
	return {
		...
		tavernHelperScriptNames: helper === void 0 ? [] : (() => {
			const root = object$5(helper, "data.extensions.tavern_helper");
			...
			return root.scripts.flatMap((value, index) => {
				const script = object$5(value, `data.extensions.tavern_helper.scripts[${index}]`);
...
function parseLorebookEntry(value, index, version) {
	...
	const extensions = object$5(entry.extensions, `${path}.extensions`);
...
function parseLorebook(value, version) {
	if (value === void 0) return void 0;
	const book = object$5(value, "data.character_book");
...
function validateVersionFields(data, version) {
	...
	object$5(data.extensions, "data.extensions");
...
function parseCharacterCardJson(json) {
	...
	const { version, specVersion, data } = cardVersion(object$5(raw, "character card"));
...
function parseEntry(value, id, degradations) {
	...
	const entry = object$4(value, path);
...
function parseWorldInfoJson(json) {
	...
	const root = object$4(raw, "World Info");
...
function parseHeader(raw, line) {
	const header = object$3(raw, `SillyTavern chat line ${line}`);
	...
	object$3(metadata, `SillyTavern chat line ${line}.chat_metadata`);
...
function parseMessage(raw, line) {
	...
	const message = object$3(raw, path);
...
function prompt(value, index) {
	const record = object$2(value, `prompts[${index}]`);
...
function selectedOrder(value) {
	...
	const rows = value.map((entry, index) => object$2(entry, `prompt_order[${index}]`));
	...
	return selected.order.map((entry, index) => {
		const record = object$2(entry, `prompt_order.order[${index}]`);
		const identifier = text(record.identifier).trim();
...
function parseSillyTavernPresetJson(source, fileName = "SillyTavern preset") {
	...
	const record = object$2(value, "preset");
	const prompts = record.prompts.map(prompt);
	...
	const extensions = record.extensions === void 0 ? {} : object$2(record.extensions, "extensions");
	return {
		format: 0,
...
function order(value) {
	...
	return value.map((item, index) => {
		const record = object$1(item, `order[${index}]`);
...
function generation(value) {
	const record = object$1(value, "generation");
	return {
...
function parsePresetConfigurationRequest(source) {
	...
	const value = object$1(parsed, "preset configuration");
...
function createPresetSessionSeed(events, preset, attachment) {
...
function presetProjection(name, preset, revision) {
