...
function parseRegexScript(value, path) {
	const script = object$9(value, path);
	return {
...
function parseFrontend(data) {
	...
	return {
		...
		tavernHelperScriptNames: helper === void 0 ? [] : (() => {
			const root = object$8(helper, "data.extensions.tavern_helper");
			...
			return root.scripts.flatMap((value, index) => {
				const script = object$8(value, `data.extensions.tavern_helper.scripts[${index}]`);
...
function parseLorebookEntry(value, index, version) {
	...
	const extensions = object$8(entry.extensions, `${path}.extensions`);
...
function parseLorebook(value, version) {
	if (value === void 0) return void 0;
	const book = object$8(value, "data.character_book");
...
function validateVersionFields(data, version) {
	...
	object$8(data.extensions, "data.extensions");
...
function parseCharacterCardJson(json) {
	...
	const { version, specVersion, data } = cardVersion(object$8(raw, "character card"));
...
function parseWorldInfoJson(json) {
	...
	const root = object$7(raw, "World Info");
...
function parseHeader(raw, line) {
	const header = object$6(raw, `SillyTavern chat line ${line}`);
	...
	object$6(metadata, `SillyTavern chat line ${line}.chat_metadata`);
...
function parseMessage(raw, line) {
	...
	const message = object$6(raw, path);
...
function prompt$1(value, index) {
	const record = object$5(value, `prompts[${index}]`);
...
function selectedOrder(value) {
	...
	const rows = value.map((entry, index) => object$5(entry, `prompt_order[${index}]`));
	...
	return selected.order.map((entry, index) => {
		const record = object$5(entry, `prompt_order.order[${index}]`);
		const identifier = text(record.identifier).trim();
...
function parseSillyTavernPresetJson(source, fileName = "SillyTavern preset") {
	...
	const record = object$5(value, "preset");
	...
	const extensions = record.extensions === void 0 ? {} : object$5(record.extensions, "extensions");
	const rawRegex = extensions.regex_scripts;
	const regexScripts = rawRegex === void 0 ? [] : (() => {
...
function order(value) {
	...
	return value.map((item, index) => {
		const record = object$4(item, `order[${index}]`);
...
function regex$1(value) {
	...
	return value.map((item, itemIndex) => {
		const record = object$4(item, `regex[${itemIndex}]`);
...
function content(value) {
	...
	const seen = /* @__PURE__ */ new Set();
	return value.map((item, itemIndex) => {
		const record = object$4(item, `content[${itemIndex}]`);
...
function promptDefinitions(value) {
	...
	const seen = /* @__PURE__ */ new Set();
	return value.map((item, itemIndex) => {
		const record = object$4(item, `prompts[${itemIndex}]`);
...
function generation(value) {
	const record = object$4(value, "generation");
	return {
...
function parsePresetConfigurationRequest(source) {
	...
	const value = object$4(parsed, "preset configuration");
...
function parseMeta(value) {
	const meta = object$3(value, "import_sillytavern_preset metadata");
	const result = object$3(meta.result, "import_sillytavern_preset result");
	const preset = object$3(meta.preset, "import_sillytavern_preset preset");
...
function parseGenerationRequest(source) {
	...
	const request = object$2(value, "回复操作请求");
...
const agentRpProjectionDefinition = {
	...
	apply(state, event) {
		...
		if (event.type === "assistant/message" && event.surfaceOp === "append") {
			const text = event.data.message.content.flatMap((block) => block.type === "text" ? [block.text] : []).join("\n");
			const nextState = text.trim() === "" ? state : {
				...state,
				currentReplySeq: event.seq
			};
			if (state.mvu === void 0 || !/<UpdateVariable(?:variable)?>/iu.test(text)) return nextState;
			try {
