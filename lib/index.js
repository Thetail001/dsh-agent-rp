...
function parseFrontend(data) {
	...
	const regexScripts = rawRegex === void 0 ? [] : (() => {
		...
		return rawRegex.map((value, index) => {
			...
			const script = object$2(value, path);
			return {
	...
	return {
		...
		tavernHelperScriptNames: helper === void 0 ? [] : (() => {
			const root = object$2(helper, "data.extensions.tavern_helper");
			...
			return root.scripts.flatMap((value, index) => {
				const script = object$2(value, `data.extensions.tavern_helper.scripts[${index}]`);
...
function parseLorebookEntry(value, index, version) {
	...
	const extensions = object$2(entry.extensions, `${path}.extensions`);
...
const agentRpProjectionDefinition = {
	...
	apply(state, event) {
		...
		if (event.type === "tool/call") {
			const kind = event.data.name === "import_character_card" ? "character-card" : event.data.name === "import_world_info" ? "world-info" : void 0;
...
async function requestSupplement(ctx, options, current, rules, assistantReply) {
	...
	ctx.on("llm/stream", (options, next) => {
		...
		return (async function* () {
			...
			try {
				...
				const normalized = supplemental.text === void 0 ? void 0 : normalizeMvuSupplement(current.statData, supplemental.text);
