...
function parseRegexScript(value, path) {
	const script = object$7(value, path);
	return {
...
function parseFrontend(data) {
	const extensions = optionalObject$1(data.extensions, "data.extensions");
	...
	return {
		...
		tavernHelperScriptNames: helper === void 0 ? [] : (() => {
			const root = object$6(helper, "data.extensions.tavern_helper");
			...
			return root.scripts.flatMap((value, index) => {
				const script = object$6(value, `data.extensions.tavern_helper.scripts[${index}]`);
...
function parseLorebookEntry(value, index, version) {
	...
	const extensions = object$6(entry.extensions, `${path}.extensions`);
...
function parseLorebook(value, version) {
	if (value === void 0) return void 0;
	const book = object$6(value, "data.character_book");
	optionalObject$1(book.extensions, "data.character_book.extensions");
	...
	const extensionTokenBudget = optionalFiniteNumber$1(optionalObject$1(book.extensions, "data.character_book.extensions")?.token_budget, "data.character_book.extensions.token_budget");
...
function validateVersionFields(data, version) {
	...
	object$6(data.extensions, "data.extensions");
...
function parseCharacterCardJson(json) {
	...
	const { version, specVersion, data } = cardVersion(object$6(raw, "character card"));
...
function parseEntry(value, id, degradations) {
	...
	const entry = object$5(value, path);
...
function parseWorldInfoJson(json) {
	...
	const root = object$5(raw, "World Info");
...
function parseHeader(raw, line) {
	const header = object$4(raw, `SillyTavern chat line ${line}`);
	...
	object$4(metadata, `SillyTavern chat line ${line}.chat_metadata`);
...
function parseMessage(raw, line) {
	...
	const message = object$4(raw, path);
...
function prompt$1(value, index) {
	const record = object$3(value, `prompts[${index}]`);
...
function selectedOrder(value) {
	...
	const rows = value.map((entry, index) => object$3(entry, `prompt_order[${index}]`));
	...
	return selected.order.map((entry, index) => {
		const record = object$3(entry, `prompt_order.order[${index}]`);
		const identifier = text(record.identifier).trim();
...
function parseSillyTavernPresetJson(source, fileName = "SillyTavern preset") {
	...
	const record = object$3(value, "preset");
	...
	const extensions = record.extensions === void 0 ? {} : object$3(record.extensions, "extensions");
	const rawRegex = extensions.regex_scripts;
	const regexScripts = rawRegex === void 0 ? [] : (() => {
...
function order(value) {
	...
	return value.map((item, index) => {
		const record = object$2(item, `order[${index}]`);
...
function regex$1(value) {
	...
	return value.map((item, itemIndex) => {
		const record = object$2(item, `regex[${itemIndex}]`);
...
function content(value) {
	...
	const seen = /* @__PURE__ */ new Set();
	return value.map((item, itemIndex) => {
		const record = object$2(item, `content[${itemIndex}]`);
...
function promptDefinitions(value) {
	...
	const seen = /* @__PURE__ */ new Set();
	return value.map((item, itemIndex) => {
		const record = object$2(item, `prompts[${itemIndex}]`);
...
function generation(value) {
	const record = object$2(value, "generation");
	return {
...
function parsePresetConfigurationRequest(source) {
	...
	const value = object$2(parsed, "preset configuration");
...
function parseMeta(value) {
	const meta = object$1(value, "import_sillytavern_preset metadata");
	const result = object$1(meta.result, "import_sillytavern_preset result");
	const preset = object$1(meta.preset, "import_sillytavern_preset preset");
...
function summary(id, name, preset, updatedAt) {
	const enabled = new Set(preset.order.filter((item) => item.enabled).map((item) => item.identifier));
	return {
		id,
		name,
...
var PresetLibrary = class {
	...
	readFile(path) {
		...
		return {
			...summary(meta.id, meta.name, preset, meta.updatedAt),
			preset
		};
	}
		...
		text: ({ scope }) => {
			...
			if (preset !== void 0) {
				...
				const assembled = assembleSillyTavernPreset(preset, {
					card,
					...presetUserName === void 0 ? {} : { userName: presetUserName },
			...
			return renderImportedCharacterPrompt(card, [...standaloneLore.beforeCharacter, ...characterLore.beforeCharacter], [...characterLore.afterCharacter, ...standaloneLore.afterCharacter], readSillyTavernChatIdentity(agent.session.events)?.userName, mvu !== void 0);
		},
		complete: true
	});
...
export { CHARACTER_IMPORT_VALUE_SCHEMA, Config, MEMORY_VALUE_SCHEMA, PRESET_IMPORT_VALUE_SCHEMA, WORLD_INFO_IMPORT_VALUE_SCHEMA, apply, claimAgentRpPrompt, inject, installAgentRp, isCharacterCardSessionOffer, isSillyTavernChatOffer, isSillyTavernMigrationOffer, isSillyTavernPresetOffer, name };
