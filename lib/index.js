...
function claimCharacterCardPrompt(agentRpActive, content) {
	if (!agentRpActive) return void 0;
	const cards = content.filter((part) => part.type !== "text").filter(isCharacterCardOffer);
	const text = content.filter((part) => part.type === "text").map((part) => part.text).join("\n");
	if (cards.length !== 1 || !/(?:角色卡|character\s*card|导入|接管|切换角色)/iu.test(text)) return void 0;
	return { text };
}
...
function installAgentRp(ctx, config) {
	...
	ctx.effect(() => gateway.registerPromptAttachmentConsumer("dsh-agent-rp", ({ agent, content }) => claimCharacterCardPrompt(agentsByScope.get(agent) === agent, content)), "agent-rp: prompt attachment consumer");
...
export { CHARACTER_IMPORT_VALUE_SCHEMA, Config, MEMORY_VALUE_SCHEMA, apply, claimCharacterCardPrompt, inject, installAgentRp, name };
