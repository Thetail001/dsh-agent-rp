...
*/
function createCharacterCardSessionSeed(card, attachment, greetingIndex, renderedGreeting) {
	const { raw, ...result } = prepareCharacterImportResult(card, { transport: "json" }, 0, attachment, greetingIndex);
	const meta = {
		format: 0,
		result,
		raw
...
* @param cardAttachment - stored card JSON.
...
*/
function createSillyTavernMigrationSeed(card, cardAttachment, chat, chatAttachment) {
	const events = [...createSillyTavernChatSeed(chat, chatAttachment)];
	const cardEvent = createCharacterCardSessionSeed(card, cardAttachment, 0, "")[0];
...
function isSillyTavernMigrationOffer(agentRpActive, content) {
	...
	return content.filter((part) => part.type !== "text").length === 2 && files.filter((part) => /\.json$/iu.test(part.name)).length === 1 && files.filter((part) => /\.jsonl$/iu.test(part.name)).length === 1;
}
...
function isCharacterCardSessionOffer(agentRpActive, content) {
	...
	const text = content.filter((part) => part.type === "text").map((part) => part.text).join("\n");
	const attachments = content.filter((part) => part.type !== "text");
	return /^请导入这张角色卡$/u.test(text.trim()) && attachments.length === 1 && attachments[0]?.type === "file" && /\.json$/iu.test(attachments[0].name);
}
...
function installAgentRp(ctx, config) {
	...
	ctx.effect(() => gateway.registerPromptSessionImporter("dsh-agent-rp:sillytavern-migration", {
		...
		async import(input, signal) {
			...
			if (cardAttachment === void 0 || chatAttachment === void 0) throw new Error("SillyTavern migration requires one Character Card JSON and one chat JSONL");
			const [cardBytes, chatBytes] = await Promise.all([input.readFile(cardAttachment, signal), input.readFile(chatAttachment, signal)]);
			const card = parseCharacterCardJsonBytes(cardBytes);
			return {
				seed: createSillyTavernMigrationSeed(card, cardAttachment, parseSillyTavernChatBytes(chatBytes), chatAttachment),
	...
	ctx.effect(() => gateway.registerPromptSessionImporter("dsh-agent-rp:character-card-json", {
		...
		async import(input, signal) {
			...
			const attachment = input.attachments[0];
			if (attachment === void 0 || !/\.json$/iu.test(attachment.name)) throw new Error("Character Card import requires one .json file");
			const card = parseCharacterCardJsonBytes(await input.readFile(attachment, signal));
			return {
				seed: createCharacterCardSessionSeed(card, attachment, 0, substituteCardMacros(card.firstMessage, card)),
				title: card.nickname?.trim() || card.name
			};
		}
