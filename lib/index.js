...
function installAgentRp(ctx, config) {
	...
	ctx.effect(() => gateway.registerPromptSessionImporter("dsh-agent-rp:sillytavern-chat", {
		...
		async import(input, signal) {
			...
			if (attachment === void 0 || !/\.jsonl$/iu.test(attachment.name)) throw new Error("SillyTavern chat import requires one .jsonl file");
...
export { CHARACTER_IMPORT_VALUE_SCHEMA, Config, MEMORY_VALUE_SCHEMA, WORLD_INFO_IMPORT_VALUE_SCHEMA, apply, claimAgentRpPrompt, inject, installAgentRp, isSillyTavernChatOffer, name };
