...
function parseRegexScript(value, path) {
	const script = object$8(value, path);
	return {
...
function parseFrontend(data) {
	...
	return {
		...
		tavernHelperScriptNames: helper === void 0 ? [] : (() => {
			const root = object$7(helper, "data.extensions.tavern_helper");
			...
			return root.scripts.flatMap((value, index) => {
				const script = object$7(value, `data.extensions.tavern_helper.scripts[${index}]`);
...
function parseLorebookEntry(value, index, version) {
	...
	const entry = object$7(value, path);
	const extensions = object$7(entry.extensions, `${path}.extensions`);
...
function parseLorebook(value, version) {
	if (value === void 0) return void 0;
	const book = object$7(value, "data.character_book");
...
function validateVersionFields(data, version) {
	...
	object$7(data.extensions, "data.extensions");
...
function parseCharacterCardJson(json) {
	...
	const { version, specVersion, data } = cardVersion(object$7(raw, "character card"));
...
function parseEntry(value, id, degradations) {
	...
	const entry = object$6(value, path);
...
function parseWorldInfoJson(json) {
	...
	const root = object$6(raw, "World Info");
...
function parseHeader(raw, line) {
	const header = object$5(raw, `SillyTavern chat line ${line}`);
	...
	object$5(metadata, `SillyTavern chat line ${line}.chat_metadata`);
...
function parseMessage(raw, line) {
	...
	const message = object$5(raw, path);
...
function prompt$1(value, index) {
	const record = object$4(value, `prompts[${index}]`);
	...
	return {
		...
		...optionalFinite(record.injection_position, `prompts[${index}].injection_position`) === void 0 ? {} : { injectionPosition: record.injection_position },
		...optionalFinite(record.injection_depth, `prompts[${index}].injection_depth`) === void 0 ? {} : { injectionDepth: record.injection_depth },
		...optionalFinite(record.injection_order, `prompts[${index}].injection_order`) === void 0 ? {} : { injectionOrder: record.injection_order }
	};
}
function selectedOrder(value) {
	...
	const rows = value.map((entry, index) => object$4(entry, `prompt_order[${index}]`));
	...
	return selected.order.map((entry, index) => {
		const record = object$4(entry, `prompt_order.order[${index}]`);
		const identifier = text(record.identifier).trim();
...
function parseSillyTavernPresetJson(source, fileName = "SillyTavern preset") {
	...
	const record = object$4(value, "preset");
	...
	const extensions = record.extensions === void 0 ? {} : object$4(record.extensions, "extensions");
	const rawRegex = extensions.regex_scripts;
	const regexScripts = rawRegex === void 0 ? [] : (() => {
	...
	return {
		...
		order,
		generation: {
			...optionalFinite(record.temperature, "temperature") === void 0 ? {} : { temperature: record.temperature },
			...optionalFinite(record.openai_max_tokens, "openai_max_tokens") === void 0 ? {} : { maxTokens: record.openai_max_tokens },
			...typeof record.reasoning_effort === "string" ? { reasoningEffort: record.reasoning_effort } : {},
			...optionalFinite(record.top_p, "top_p") === void 0 ? {} : { topP: record.top_p },
			...optionalFinite(record.top_k, "top_k") === void 0 ? {} : { topK: record.top_k },
			...optionalFinite(record.top_a, "top_a") === void 0 ? {} : { topA: record.top_a },
			...optionalFinite(record.min_p, "min_p") === void 0 ? {} : { minP: record.min_p },
			...optionalFinite(record.frequency_penalty, "frequency_penalty") === void 0 ? {} : { frequencyPenalty: record.frequency_penalty },
			...optionalFinite(record.presence_penalty, "presence_penalty") === void 0 ? {} : { presencePenalty: record.presence_penalty },
			...optionalFinite(record.repetition_penalty, "repetition_penalty") === void 0 ? {} : { repetitionPenalty: record.repetition_penalty }
		},
		formats: {
...
function order(value) {
	...
	return value.map((item, index) => {
		const record = object$3(item, `order[${index}]`);
...
function regex$1(value) {
	...
	return value.map((item, itemIndex) => {
		const record = object$3(item, `regex[${itemIndex}]`);
...
function content(value) {
	...
	const seen = /* @__PURE__ */ new Set();
	return value.map((item, itemIndex) => {
		const record = object$3(item, `content[${itemIndex}]`);
...
function promptDefinitions(value) {
	...
	const seen = /* @__PURE__ */ new Set();
	return value.map((item, itemIndex) => {
		const record = object$3(item, `prompts[${itemIndex}]`);
...
function generation(value) {
	const record = object$3(value, "generation");
	return {
...
function parsePresetConfigurationRequest(source) {
	...
	const value = object$3(parsed, "preset configuration");
...
function parseMeta(value) {
	const meta = object$2(value, "import_sillytavern_preset metadata");
	const result = object$2(meta.result, "import_sillytavern_preset result");
	const preset = object$2(meta.preset, "import_sillytavern_preset preset");
...
const IMPORT_BEHAVIOR = "用户附带 SillyTavern 角色卡 PNG 或 JSON 并要求导入、接管或切换角色时，调用 import_character_card；附带独立 World Info / 世界书 JSON 并要求导入时，调用 import_world_info；附带 Chat Completion 预设 JSON 并要求导入时，调用 import_sillytavern_preset。一条消息附有多个同类文件时才指定从零开始的 attachmentIndex。导入成功后直接采用新角色、世界设定或预设，不解释内部格式。";
/**
...
* @returns model-visible system prompt text.
*/
function renderImportedCharacterPrompt(card, loreBefore, loreAfter, userName, mvuEnabled = false) {
	const name = card.nickname?.trim() || card.name;
...
const agentRpProjectionDefinition = {
	...
	apply(state, event) {
		if (event.type === "agent-rp/sillytavern-chat-import") {
			...
			return {
				...
				character: {
					...state.character,
					...state.character.source === "preset" && identity !== void 0 ? {
		...
		if (event.type === "command/done" && event.data.kind === "success") {
			...
			if (library === void 0) return state;
			...
			if (state.preset === void 0 || state.presetState === void 0 || library.linkedLibraryId === void 0) return {
				...state,
				presetLibrary: library.entries
			};
			return {
		...
		if (event.type === "command/run" && event.data.name === "rp-preset-configure" && event.data.args !== void 0) {
			if (state.preset === void 0 || state.presetState === void 0) return state;
			try {
				...
				return {
					...state,
					preset: presetProjection(state.preset.name, configured, revision, state.presetState.importedPreset, state.presetState.libraryId),
					presetState: {
						...state.presetState,
						preset: configured,
						revision
	...
	view: (state) => ({
		...state.character,
		worldInfoCount: state.cardWorldInfoCount + Object.values(state.standaloneWorldInfos).reduce((total, count) => total + count, 0),
...
function parseMetadata(value) {
	...
	const validTransport = meta.transport === "json" ? meta.metadataKeyword === void 0 : meta.transport === "png" && (meta.metadataKeyword === "ccv3" || meta.metadataKeyword === "chara");
	if (meta.format !== 0 || typeof meta.id !== "string" || !ID_PATTERN.test(meta.id) || typeof meta.originalFilename !== "string" || meta.originalFilename.trim() === "" || typeof meta.mediaType !== "string" || meta.mediaType.trim() === "" || !validTransport || typeof meta.bytes !== "number" || !Number.isSafeInteger(meta.bytes) || meta.bytes < 1 || typeof meta.createdAt !== "number" || !Number.isSafeInteger(meta.createdAt) || meta.createdAt < 0 || typeof meta.updatedAt !== "number" || !Number.isSafeInteger(meta.updatedAt) || meta.updatedAt < 0) throw new Error("character library metadata has invalid fields");
	return meta;
}
...
function summary(meta, card) {
	return {
		...
		avatarAvailable: meta.transport === "png",
		transport: meta.transport,
		updatedAt: meta.updatedAt
	};
}
/** Small content-addressed card library; the original PNG or JSON remains exportable. */
var CharacterLibrary = class {
	...
	list() {
		if (!existsSync(this.root)) return [];
		return readdirSync(this.root).filter((filename) => filename.endsWith(META_SUFFIX)).map((filename) => this.readEntry(join(this.root, filename)).summary).sort((left, right) => right.updatedAt - left.updatedAt || left.displayName.localeCompare(right.displayName));
	}
	/** Load card metadata and selectable greetings by opaque id. */
	get(id) {
		const entry = this.readId(id);
		const card = this.parseCard(entry.meta, entry.data);
		return {
			...summary(entry.meta, card),
			originalFilename: entry.meta.originalFilename,
			mediaType: entry.meta.mediaType,
			greetings: [card.firstMessage, ...card.alternateGreetings]
		};
	}
	...
	asset(id) {
		...
		const card = this.parseCard(entry.meta, entry.data);
		return {
			summary: summary(entry.meta, card),
			originalFilename: entry.meta.originalFilename,
			mediaType: entry.meta.mediaType,
			data: entry.data
		};
	...
	import(input) {
		...
		const meta = {
			...
			mediaType: input.mediaType?.trim() || (transport === "png" ? "image/png" : "application/json"),
			transport,
	...
	parseCard(meta, data) {
		if (meta.transport === "json") return parseCharacterCardJsonBytes(data);
		...
		return parseCharacterCardJson(payload.json);
	}
	readId(id) {
...
function installCharacterLibraryHttp(ctx, library) {
	ctx.effect(() => ctx.httpServer.register({
		...
		handler(request, response) {
			...
			try {
				...
				if (parts.length === 1 && parts[0] !== void 0) {
					json(response, 200, {
						format: 0,
				...
				if (parts.length === 2 && parts[0] !== void 0 && parts[1] === "asset") {
...
function parseCharacterCardSessionRequest(text) {
	...
	if (record.format !== 0 || typeof record.greetingIndex !== "number" || !Number.isSafeInteger(record.greetingIndex) || record.greetingIndex < 0 || record.userName !== void 0 && (typeof record.userName !== "string" || record.userName.trim() === "" || record.userName.trim().length > 120) || keys.some((key) => key !== "format" && key !== "greetingIndex" && key !== "userName")) return void 0;
	return {
		format: 0,
...
function installAgentRp(ctx, config, options = {}) {
	...
	if (registerSessionImporter !== void 0) ctx.effect(() => registerSessionImporter("dsh-agent-rp:character-card", {
		...
		async import(input, signal) {
			...
			return {
				seed: createCharacterCardSessionSeed(card, stored.ref, request.greetingIndex, greeting, transport, request.userName),
				title: card.nickname?.trim() || card.name
			};
		}
	...
	ctx.systemPrompt.section({
		...
		text: ({ scope }) => {
			...
			return renderImportedCharacterPrompt(card, [...standaloneLore.beforeCharacter, ...characterLore.beforeCharacter], [...characterLore.afterCharacter, ...standaloneLore.afterCharacter], userName, mvu !== void 0);
		},
		complete: true
	});
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (isJsonCharacterCardAttachment(attachment)) {
				...
				return prepareCharacterImportResult(card, { transport: "json" }, direct.eventSeq, stored.ref, args.greetingIndex ?? 0, readSillyTavernChatIdentity(exec.agent.session.events)?.userName);
			}
