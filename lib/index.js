...
function stringArray$1(value, path, fallback = []) {
...
function parseLorebookEntry(value, index, version) {
	...
	object$1(entry.extensions, `${path}.extensions`);
	...
	const position = optionalString$1(entry.position, `${path}.position`) ?? "after_char";
	...
	return {
		keys: stringArray$1(entry.keys, `${path}.keys`),
		secondaryKeys: stringArray$1(entry.secondary_keys, `${path}.secondary_keys`),
		content,
		enabled,
		insertionOrder,
...
function parseLorebook(value, version) {
	if (value === void 0) return void 0;
	const book = object$1(value, "data.character_book");
	object$1(book.extensions, "data.character_book.extensions");
	...
	const name = optionalString$1(book.name, "data.character_book.name");
	return {
		...name === void 0 ? {} : { name },
...
function validateVersionFields(data, version) {
	...
	stringArray$1(data.alternate_greetings, "data.alternate_greetings");
	stringArray$1(data.tags, "data.tags");
	object$1(data.extensions, "data.extensions");
	if (version === 3) stringArray$1(data.group_only_greetings, "data.group_only_greetings");
}
function degradationSet(data, version, specVersion, lorebook) {
	...
	if (stringArray$1(data.group_only_greetings, "data.group_only_greetings").length > 0) result.add("group-greetings");
...
function parseCharacterCardJson(json) {
	...
	const { version, specVersion, data } = cardVersion(object$1(raw, "character card"));
	validateVersionFields(data, version);
	const lorebook = parseLorebook(data.character_book, version);
	const nickname = optionalString$1(data.nickname, "data.nickname");
	const alternateGreetings = stringArray$1(data.alternate_greetings, "data.alternate_greetings");
	const systemPrompt = optionalString$1(data.system_prompt, "data.system_prompt") ?? "";
	const postHistoryInstructions = optionalString$1(data.post_history_instructions, "data.post_history_instructions") ?? "";
	return {
		format: 0,
		version,
		specVersion,
...
function parseResult$1(value) {
	...
	if (record.version !== 0 || typeof record.name !== "string" || record.cardVersion !== 1 && record.cardVersion !== 2 && record.cardVersion !== 3 || typeof record.sourceEventSeq !== "number" || !Number.isSafeInteger(record.sourceEventSeq) || typeof record.sourceAttachmentId !== "string" || !validTransport || typeof record.greetingIndex !== "number" || !Number.isSafeInteger(record.greetingIndex) || record.greetingIndex < 0 || typeof record.selectedGreeting !== "string" || !Array.isArray(record.degradations) || record.degradations.some((value) => typeof value !== "string" || !CHARACTER_IMPORT_DEGRADATIONS.includes(value))) throw new Error("import_character_card result has invalid fields");
	return record;
}
...
*/
function prepareCharacterImportResult(card, transport, sourceEventSeq, attachment, greetingIndex) {
...
function installAgentRp(ctx, config) {
	...
	ctx.systemPrompt.section({
		...
		text: ({ scope }) => {
			...
			if (active === void 0) return renderCharacterPrompt(config, standaloneLore.beforeCharacter, standaloneLore.afterCharacter);
			const card = cardFromImportMeta(active.meta);
			const characterLore = renderImportedLorebook(card, agent.session, pendingMessages);
			return renderImportedCharacterPrompt(card, [...standaloneLore.beforeCharacter, ...characterLore.beforeCharacter], [...characterLore.afterCharacter, ...standaloneLore.afterCharacter]);
		},
		complete: true
	});
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (isJsonCharacterCardAttachment(attachment)) {
				...
				return prepareCharacterImportResult(parseCharacterCardJsonBytes(stored.data), { transport: "json" }, direct.eventSeq, stored.ref, args.greetingIndex ?? 0);
			}
...
export { CHARACTER_IMPORT_VALUE_SCHEMA, Config, MEMORY_VALUE_SCHEMA, WORLD_INFO_IMPORT_VALUE_SCHEMA, apply, claimAgentRpPrompt, inject, installAgentRp, name };
