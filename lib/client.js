...
async function __dshGenerate(mode,config){var value=__dshClone(config??{});value.__dsh_generation_mode=mode;void __dshEmitLocal(window.iframe_events.GENERATION_STARTED,[]);var promptEvents=[window.tavern_events.CHAT_COMPLETION_PROMPT_READY,window.tavern_events.GENERATE_AFTER_DATA,window.tavern_events.GENERATE_AFTER_COMBINE_PROMPTS];if(promptEvents.some(__dshHasPromptListener)){var prompts=await __dshPromptPreview(mode,__dshGenerationConfig(__dshGenerationBody(value)));await __dshEmitPromptPreview(prompts,value)}var response=await window.fetch('/api/backends/chat-completions/generate',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(__dshGenerationBody(value))});var raw=await response.text();var result;try{result=JSON.parse(raw)}catch(error){throw new Error(response.ok?'模型返回了无法识别的结果':'生成失败（'+response.status+'）')}if(!response.ok)throw new Error(__dshGenerationError(result));var text=__dshGenerationText(result);if(value.should_stream===true){void __dshEmitLocal(window.iframe_events.STREAM_TOKEN_RECEIVED_FULLY,[text]);void __dshEmitLocal(window.iframe_events.STREAM_TOKEN_RECEIVED_INCREMENTALLY,[text])}void __dshEmitLocal(window.iframe_events.GENERATION_ENDED,[text]);return text}
...
window.SillyTavern={chat:[],name1:__dshSnapshot.userName??'用户',name2:__dshSnapshot.characterName,characterId:__dshSnapshot.characterId,chatId:__dshSnapshot.chatId,chatMetadata:__dshCurrentChatMetadata,chat_metadata:__dshCurrentChatMetadata,extensionSettings:{},getCurrentCharacterId:window.getCurrentCharId,getCurrentChatId:window.getCurrentChatId,eventSource:{on:window.eventOn,once:window.eventOnce,emit:window.eventEmit,emitAndWait:window.eventEmitAndWait,removeListener:window.eventRemoveListener},eventTypes:window.tavern_events,getContext:function(){return this}};
				...
				const bridge = (event) => {
					...
					if (message.action === "trigger-slash" && typeof message.requestId === "string" && typeof message.value === "string" && message.value.length <= 65536) {
						...
						if (command?.kind === "send" || command?.kind === "set-input") {
							...
							else Promise.resolve(conversation.send(command.text)).then(resolve, reject);
							return;
						}
						if (command?.kind === "trigger") {
							runTrigger(sessionId).then(resolve, reject);
							return;
						}
