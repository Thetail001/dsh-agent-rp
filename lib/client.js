...
Please report this to https://github.com/markedjs/marked.`, e) {
		...
		function tavernMessageDepth(messages, messageId) {
			if (messages === void 0 || messageId === void 0) return void 0;
			const index = messages.findIndex((message) => message.messageId === messageId);
			return index < 0 ? void 0 : messages.length - index - 1;
		}
		...
		*/
		...
		function runtimeSource(snapshot, compatibilityMarkers) {
			return `
'use strict';
...
`;
		...
		function tavernScriptFrameSource(script, execution, snapshot) {
			...
			const preloads = plan.preloads.map((preload) => preload === "zod" ? `import(${safeJson(ZOD_MODULE_URL)}).then(function(module){window.z=module})` : `import(${safeJson(YAML_MODULE_URL)}).then(function(module){window.YAML=module.default??module})`);
			...
			const bootstrap = `void (async function(){try{${preloads.length === 0 ? "" : `await Promise.all([${preloads.join(",")}]);`}${execute};__dshPost('ready',{markers:__dshCompatibilityMarkers()})}catch(error){console.error(error);__dshPost('runtime-error',{value:String(error)})}})();`;
			return `<!doctype html><html><head><meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'unsafe-inline' 'unsafe-eval' blob: ${origins}; connect-src 'none'; img-src 'none'; style-src 'unsafe-inline'; font-src 'none'; frame-src 'none'">${libraries}<style>html,body{background:transparent;color-scheme:dark}</style></head><body><script>${runtimeSource(snapshot, plan.compatibilityMarkers)}\n${bootstrap}<\/script></body></html>`;
		}
		//#endregion
		...
		function blockedCardFrameOrigins(segmentOrigins, character) {
...
img,svg,video,canvas{max-width:100%}
		...
		function boundedCompatibilityMarkers(value) {
			if (value === void 0) return [];
			return [...new Set(value.filter((marker) => marker.length <= 128 && compatibilityMarkerPattern.test(marker)))].sort().slice(0, 32);
		}
		function mvuFrameRuntime(statData, compatibilityMarkers) {
			return `
var __dshStatData=${JSON.stringify(statData ?? {}).replace(/</gu, "\\u003c").replace(/\u2028/gu, "\\u2028").replace(/\u2029/gu, "\\u2029")};
var __dshCompatibilityMarkers=${JSON.stringify(boundedCompatibilityMarkers(compatibilityMarkers))};
...
`;
		...
		function redirectKnownHostFacades(source) {
			const localHostAliases = source.replace(/(?:window\s*\.\s*)?(?:parent|top)\s*(?:\|\||\?\?)\s*window\b/gu, "window");
		...
		function inlineCardFrameSource(source, options) {
			...
			const markdown = k$1.parse(legacy.source, {
				async: false,
				breaks: true,
				gfm: true
			});
			return {
				srcDoc: cardFrameSource(purify.sanitize(markdown, {
					ADD_TAGS: ["style"],
					FORBID_ATTR: ["srcdoc"],
					FORBID_TAGS: [
						"base",
		...
		*/
		...
		function notifyCharacterLibraryChanged(id) {
			...
			(0, react.useEffect)(() => {
				...
				const load = () => {
					...
					fetchCharacterDetail(libraryId).then((value) => {
						...
						if (current && requestedRevision === revision) setDetail(void 0);
					});
				};
				setDetail(void 0);
				load();
		...
		function CharacterDisplay({ compilation, statData, characterName, character, compatibilityMarkers, onReady, preview = false }) {
			const compiled = (0, react.useMemo)(() => compileCardFrames(compilation, {
				origin: window.location.origin,
				...statData === void 0 ? {} : { statData },
				...character === void 0 ? {} : { character },
				...compatibilityMarkers === void 0 ? {} : { compatibilityMarkers }
			}), [
				character,
...
`;
		...
		function worldInfoReason(entry) {
			switch (entry.reason) {
				...
				case "regex-runtime-unavailable": return {
					title: "正则运行时不可用",
					detail: "本次检查没有启动隔离正则运行时；该条目已跳过，重新启动 DSH 后可再检查"
				};
...
`;
		...
		function TavernScriptRuntime({ ctx, inputActions, onCompatibilityMarkersChange, onDisplayOverride, projection, runGeneration, runModelList, runMutation, runPresetConfiguration, runPromptPreview, runTrigger, sessionId }) {
			const scripts = [
			...
			const [compatibilityMarkersByScript, setCompatibilityMarkersByScript] = (0, react.useState)(() => /* @__PURE__ */ new Map());
			...
			const failedScriptIds = (0, react.useRef)(/* @__PURE__ */ new Set());
			const compatibilityMarkerTimers = (0, react.useRef)(/* @__PURE__ */ new Set());
			...
			(0, react.useEffect)(() => {
				...
				setCompatibilityMarkersByScript(/* @__PURE__ */ new Map());
			...
			(0, react.useEffect)(() => {
				onCompatibilityMarkersChange([...new Set([...compatibilityMarkersByScript.values()].flat())].sort());
				...
				const bridge = (event) => {
					...
					if (message.action === "ready") {
						...
						if (!failedScriptIds.current.has(entry.script.id)) {
							setCompatibilityMarkersByScript((current) => new Map(current).set(entry.script.id, validatedTavernCompatibilityMarkers(message.markers)));
						...
						for (const delay of [
							...
							const timer = window.setTimeout(() => {
								...
								frame.contentWindow?.postMessage({
									source: "dsh-agent-rp-host",
									action: "compatibility-markers-request"
								}, "*");
							}, delay);
		...
		function roleplayComposerDockComponent(ctx, runImageGeneration, runTavernMutation, runTavernGeneration, runTavernPromptPreview, runTavernModelList, runTavernTrigger, runPresetConfiguration) {
			return function RoleplayComposerDock({ inputActions, sessionId, useProjection, useSessions, useSession }) {
				...
				const [compatibilityMarkers, setCompatibilityMarkers] = (0, react.useState)([]);
				const rootRef = (0, react.useRef)(null);
				const characterDetail = useCharacterDetail(projection?.avatarLibraryId);
				...
				const onCompatibilityMarkersChange = (0, react.useCallback)((markers) => {
					setCompatibilityMarkers((current) => current.length === markers.length && current.every((marker, index) => marker === markers[index]) ? current : [...markers]);
				}, []);
				(0, react.useEffect)(() => {
					setDisplayOverrides(/* @__PURE__ */ new Map());
				}, [sessionId, transcriptSignature]);
				(0, react.useEffect)(() => {
					setCompatibilityMarkers([]);
				}, [sessionId]);
				(0, react.useLayoutEffect)(() => {
				...
				(0, react.useEffect)(() => {
					...
					const mountRenderedDisplay = (item, original, compilation, activeProjection, activeCharacterDetail, activeCompatibilityMarkers) => {
					...
					window.addEventListener("message", bridge);
					const scan = () => {
						const { chat: activeChat, characterDetail: activeCharacterDetail, compatibilityMarkers: activeCompatibilityMarkers, displayOverrides: activeDisplayOverrides, projection: activeProjection, viewMode: activeViewMode } = displayStateRef.current;
						...
						for (const item of scroll.querySelectorAll("[data-chat-flow-kind=\"user\"]")) {
							...
							mountRenderedDisplay(item, original, compileCharacterDisplay(rendered), activeProjection, activeCharacterDetail, activeCompatibilityMarkers);
						}
