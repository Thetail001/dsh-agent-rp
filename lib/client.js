...
`;
		...
		function BlankRoleplayLauncher({ session, sessionId, listCharacters, readCharacter, setCharacterArchived, importCharacterFile, startCharacterSession, listPersonas, savePersona }) {
		...
		function RoleplayHeader({ sessionId, useProjection, useSessions, loadAvatar, renameSession, configurePreset, importPreset, managePresetLibrary, configureWorldInfo, listCharacters, readCharacter, setCharacterArchived, importCharacterFile, startCharacterSession, listPersonas, savePersona }) {
		...
		function CharacterLibraryDialog({ currentCharacterName, listCharacters, readCharacter, setCharacterArchived, importCharacterFile, listPersonas, savePersona, onClose, onStart }) {
			...
			const importFile = (file) => {
				...
				importCharacterFile(file).then((entry) => listCharacters("active").then((value) => ({
					entry,
					value
				}))).then(({ entry, value }) => {
...
`;
		...
		function apply(ctx) {
			...
			const importCharacterFile = async (file) => {
				...
				const value = await response.json();
				if (!response.ok || value.entry === void 0) throw new Error(value.error ?? `角色卡导入失败（${response.status}）`);
				return value.entry;
			};
