...
`;
		...
		function RpDistributionBridgeSection({ listCharacters, listPresets, listPersonas, listWorldInfos, probe, transfer }) {
			...
			const group = (title, kind, entries) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				style: { marginTop: "20px" },
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						...
						children: "把 Agent RP 保存的原始角色卡、预设、Persona 和世界书复制到本机运行的 dsh-rp-distribution。源库和现有会话不会改变"
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						style: {
							alignItems: "end",
...
`;
		...
		function apply(ctx) {
			...
			const probeRpDistribution = async (target) => {
				...
				const value = await response.json();
				if (!response.ok || value.format !== 0 || typeof value.target !== "string" || typeof value.generatedAt !== "number" || typeof value.experienceCount !== "number" || typeof value.componentCount !== "number" || typeof value.capabilityCount !== "number") throw new Error(value.error ?? `模块化 RP 连接失败（${response.status}）`);
				return value;
			};
