...
`;
		...
		function TavernScriptRuntime({ ctx, inputActions, onDisplayOverride, projection, runGeneration, runModelList, runMutation, runPresetConfiguration, runPromptPreview, sessionId }) {
				...
				const bridge = (event) => {
					...
					if (message.action === "trigger-slash" && typeof message.requestId === "string" && typeof message.value === "string" && message.value.length <= 65536) {
						...
						if (command?.kind === "trigger") {
							reject(/* @__PURE__ */ new Error("当前尚不支持用 /trigger 单独唤醒角色；请使用 /send 内容 || /trigger"));
							return;
						}
		...
		function roleplayComposerDockComponent(ctx, runImageGeneration, runTavernMutation, runTavernGeneration, runTavernPromptPreview, runTavernModelList, runPresetConfiguration) {
		...
		function apply(ctx) {
			...
			ctx.slots.inject("conversation.composer.dock", () => ctx.slots.register({
				...
				id: "agent-rp-status",
				order: -100
			}, roleplayComposerDockComponent(ctx, runImageGeneration, runTavernMutation, runTavernGeneration, runTavernPromptPreview, runTavernModelList, configurePreset)));
