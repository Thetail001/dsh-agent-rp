...
`;
		...
		function tavernScriptFrameSource(script, source, snapshot) {
			...
			return `<!doctype html><html><head><meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'unsafe-inline' 'unsafe-eval' ${[.../* @__PURE__ */ new Set([...BUILT_IN_TAVERN_SCRIPT_ORIGINS, ...snapshot.approvedScriptOrigins])].map((origin) => new URL(origin).origin).join(" ")}; connect-src 'none'; img-src 'none'; style-src 'unsafe-inline'; font-src 'none'; frame-src 'none'">${[/\bDOMPurify\b/u.test(source) ? `<script src="${DOMPURIFY_SCRIPT_URL}" integrity="${DOMPURIFY_SCRIPT_INTEGRITY}" crossorigin="anonymous"><\/script>` : "", /\bFuse\b/u.test(source) ? `<script src="${FUSE_SCRIPT_URL}" integrity="${FUSE_SCRIPT_INTEGRITY}" crossorigin="anonymous"><\/script>` : ""].join("")}<style>html,body{background:transparent;color-scheme:dark}</style></head><body><script>${runtimeSource(snapshot)}\ntry{Function('localStorage','sessionStorage',${encoded})(__dshLocalStorage,__dshSessionStorage)}catch(error){console.error(error);parent.postMessage({source:'dsh-agent-rp-tavern-script',scriptId:${safeJson(script.id)},action:'runtime-error',value:String(error)},'*')}<\/script></body></html>`;
		}
		//#endregion
...
`;
		...
		function inlineCardFrameSource(source, options) {
			...
			const markdown = k.parse(legacy.source, {
				async: false,
				breaks: true,
				gfm: true
			});
		...
		*/
		...
		function CharacterDisplay({ compilation, statData, characterName, character, preview = false }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: compiled.segments.map((segment, index) => segment.kind === "markdown" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.MarkdownText, { text: segment.text }, index) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("iframe", {
		...
		function compactCharacterDisplayText(value) {
			...
			const summary = segments.filter((segment) => segment.kind === "markdown").map((segment) => {
				const source = k.parse(segment.text, {
					async: false,
					breaks: true,
					gfm: true
				});
		...
		function CharacterWorldInfoSection({ detail }) {
			...
			if (worldInfo === void 0 || worldInfo.entries.length === 0) return null;
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							...
							children: [worldInfo.entries.length, " 条"]
						}),
					...
					children: worldInfo.entries.map((entry, index) => {
								...
								children: [
									...
									selected !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
										...
										selected.degradations.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											...
											children: ["原卡仍保留但当前不执行：", selected.degradations.map(characterDegradationLabel).join("、")]
										}),
