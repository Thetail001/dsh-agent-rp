...
function __dshMessageId(value){var id=Number(String(value).replaceAll('{{lastMessageId}}',String(__dshMessages.length-1)));if(!Number.isInteger(id))return;return Math.min(__dshMessages.length-1,Math.max(0,id<0?__dshMessages.length+id:id))}
...
window.setChatMessages=function(messages){messages=Array.isArray(messages)?messages:[];var last=__dshMessages.length-1;var updates=messages.filter(function(message){return __dshMessageId(message?.message_id)===last&&message?.data!==undefined&&message?.message===undefined&&message?.role===undefined&&message?.name===undefined&&message?.is_hidden===undefined&&message?.extra===undefined&&message?.swipe_id===undefined&&message?.swipes===undefined&&message?.swipes_data===undefined&&message?.swipes_info===undefined});if(updates.length!==messages.length)return Promise.reject(new Error('当前仅支持修改最新楼层的 data 变量'));if(updates.length===0)return Promise.resolve();return __dshReplace(updates.at(-1).data??{},{type:'message'})};
window.createChatMessages=function(){return Promise.reject(new Error('当前版本还不能由脚本插入聊天楼层'))};
window.deleteChatMessages=function(){return Promise.reject(new Error('当前版本还不能由脚本删除聊天楼层'))};
window.rotateChatMessages=function(){return Promise.reject(new Error('当前版本还不能由脚本重排聊天楼层'))};
...
`;
		...
		*/
		...
		async function imageCredentialInfo() {
		...
		async function updateImageCredential(change) {
...
`;
		...
		function ImageGenerationSettingsPanel({ settings, writable, onSave }) {
			...
			(0, react.useEffect)(() => {
				let active = true;
				imageCredentialInfo().then((value) => {
			...
			const saveCredential = (change) => {
				...
				setError(void 0);
				updateImageCredential(change).then((value) => {
					setCredential(value);
					setCredentialValue("");
					setTestResult(void 0);
				}, (reason) => {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
							style: labelStyle,
							children: ["服务密钥（所有配置共用）", /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
								type: "password",
								autoComplete: "new-password",
								value: credentialValue,
								placeholder: credential?.configured === true ? `已配置${credential.source === void 0 ? "" : ` · ${credential.source}`}` : "OpenAI 必填；A1111 可留空",
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								disabled: !writable || testBusy || draft.provider === "openai" && credential?.configured !== true,
...
`;
		...
		function tavernScriptSnapshot(projection, script, approvedScriptOrigins) {
		...
		function TavernScriptRuntime({ ctx, inputActions, projection, runGeneration, runMutation, sessionId }) {
				...
				Promise.all(scripts.map(async (script) => {
					try {
						...
						return {
							script,
							source,
							srcDoc: tavernScriptFrameSource(script, source, tavernScriptSnapshot(projectionRef.current, script, scriptOrigins))
						};
					} catch (reason) {
						return {
							script,
			...
			(0, react.useEffect)(() => {
				const bridge = (event) => {
					...
					if (message.action === "worldbook-mutate" && typeof message.requestId === "string" && typeof message.request === "object" && message.request !== null && !Array.isArray(message.request)) {
						const target = event.source;
			...
			const buttons = scripts.flatMap((script) => script.buttonEnabled ? script.buttons.filter((button) => button.visible).map((button) => ({
				script,
				button
			})) : []);
		...
		function roleplayComposerDockComponent(ctx, runImageGeneration, runTavernMutation, runTavernGeneration) {
