...
`;
		...
		function cardFrameSource(source, options) {
			...
			const allowedImageOrigins = [.../* @__PURE__ */ new Set([options.origin, ...options.character?.displayExtensions.filter((extension) => extension.enabled).flatMap((extension) => extension.remoteImageOrigins) ?? []])].map((origin) => origin.replace(/["'<>\s]/gu, "")).filter(Boolean).join(" ");
