		...
		function roleplayComposerDockComponent(ctx) {
			return function RoleplayComposerDock({ inputActions, sessionId, useProjection, useSessions, useSession }) {
				...
				(0, react.useEffect)(() => {
					...
					const bridge = (event) => {
						if (![...mounted].some((frame) => frame.contentWindow === event.source) || typeof event.data !== "object" || event.data === null) return;
						const message = event.data;
						if (message.source !== "dsh-agent-rp-card" || typeof message.value !== "string" || message.value.length > 65536) return;
					...
					const scan = () => {
						...
						for (const item of scroll.querySelectorAll("[data-chat-flow-kind=\"assistant-step\"]")) {
							...
							const raw = node.data.blocks?.flatMap((block) => block.kind === "text" && block.text !== void 0 ? [block.text] : []).join("\n") ?? "";
							if (raw === "") continue;
