...
报告的 `interactions` 还提供与文案无关的真实入口状态。角色库、会话设置、预设、世界书、酒馆脚本面板、小手机与脚本权限分别使用 `data-agent-rp-action` 操作入口，用 `data-agent-rp-surface` 和 `data-agent-rp-surface-state` 确认界面已经打开或关闭。浏览器 smoke 不依赖角色名、脚本名或中文按钮文案；它只按稳定标记执行一次打开和关闭，并等待对应的 `interactions` 状态改变。角色库打开后，smoke 会执行一次收起/恢复往返：选中导入的角色（`data-agent-rp-character-id`），确认归档按钮（`data-agent-rp-character-archive-toggle`）出现，切到收纳箱分区（`data-agent-rp-collection-tab-value="archived"`）记录基线条目数，再切回角色卡分区并执行“移到收纳箱”，等待角色卡分区条目数减一、收纳箱条目数加一；随后在收纳箱里重新选中该角色并执行“移回角色库”，等待两个分区的条目数回到基线。角色库根节点用 `data-agent-rp-character-collection` 输出当前分区（`active` 或 `archived`），`interactions.characterLibrary` 的 `entries` 与 `archiveToggle` 只暴露条目数与控件存在性，不包含角色名、文件名或正文。预设管理器打开后，smoke 还会执行一次会话内预设开关的往返：点击第一个可见的 `data-agent-rp-preset-toggle` 开关，再点击 `data-agent-rp-action="save-session-preset"` 保存，然后等待 Session 预设的单调 revision 前进且恰好有一个启用计数（提示模块 `data-agent-rp-preset-enabled-count` 或正则 `data-agent-rp-preset-enabled-regex-count`）变化 ±1；管理器没有可切换开关时这一步按成功跳过，保存后产品自己关闭管理器时不再重复关闭。世界书管理器打开后，smoke 会执行一次条目开关的往返：点击第一个可见的 `data-agent-rp-world-entry` 条目，再点击 `data-agent-rp-world-entry-toggle`，然后等待世界书配置的单调 revision（`data-agent-rp-world-info-revision`）前进；管理器没有条目时按成功跳过。预设计数一旦超过对应总数（`data-agent-rp-preset-prompt-count` 或 `data-agent-rp-preset-regex-count`），统一报告加入 `preset-count-mismatch` 且 `presetCountsConsistent` 为假。会话状态根节点用 `data-agent-rp-preset-revision`、`data-agent-rp-preset-prompt-count`、`data-agent-rp-preset-enabled-count`、`data-agent-rp-preset-regex-count` 与 `data-agent-rp-preset-enabled-regex-count` 输出当前会话预设的内容无关计数，用 `data-agent-rp-world-info-revision` 输出当前会话世界书配置 revision；这些字段不包含预设名、模块名、脚本名、角色名、书名、正文或表达式。
