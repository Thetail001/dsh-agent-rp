...
默认场景约为 6 MiB Character Card V3 JSON，包含 320 条世界书、2000 个外部资源引用、24 条显示正则、Tavern Helper/MVU 初始变量，并连续执行 128 次隔离 EJS。输出分别记录解析、导入、冷启动列表、完整详情、无世界书正文的角色概览、首批世界书、原始资源读取、会话投影、轻前端显示和 EJS 批处理的时间与进程堆变化。
