...
浏览器验收前可先运行 `pnpm run audit:card -- <本地角色卡路径>`。命令支持 PNG、JSON 和 CHARX，只输出格式、数量、稳定失败类别、耗时、静态轻前端资源与唯一来源的分类计数、依赖体积和实际网络请求数；不会输出文件路径、角色名、正文、正则表达式、脚本名称、脚本源码或远程 URL。它会在隔离运行时中检查真实 EJS 与世界书正则，并为 EJS 提供角色主世界书身份与固定的可重放审计时间；按浏览器的并发启动方式读取内置受信来源的脚本依赖，但不执行脚本。需要额外来源许可或无法解析的脚本只增加对应计数。
