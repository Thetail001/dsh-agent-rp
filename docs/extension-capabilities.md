...
轻前端不获得 Tavern 脚本使用的通用 `chat.session.mutate`。其 `createChatMessages` 兼容面只映射为 `chat.user-message.append`：请求只能在末尾追加一条非空用户消息，Host 只接受由已注册兼容外壳在浏览器瞬时激活期间盖章的请求，并按 Session 串行执行卡片操作。追加成功后，Host 为同一帧签发一份 30 秒内有效且只能消费一次的生成许可，使脚本在异步保存完成后调用 `/trigger`；没有点击、追加失败、许可过期或第二次触发都不能启动模型。轻前端的本地聊天数组只是卡片自有的兼容快照，成功回执会用当前 Persona 名称追加同一条消息，但不会伪装成 Host 的完整消息编号或开放其他会话改写。
