...
import { withToolGuidance } from '../src/tool-guidance.ts'

test('makes the top-level Agent the character and permits concise silence', () => {
  const config = resolveConfig({ characterName: '小满' })
  const basePrompt = renderCharacterPrompt(config)
  const prompt = withToolGuidance(basePrompt, config.toolGuidance)
  ...
  assert.doesNotMatch(basePrompt, /先调用 remember/u)
