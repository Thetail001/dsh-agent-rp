...
test('reports card frame size through timers when offscreen iframes throttle animation frames', () => {
  ...
  assert.equal(frame?.kind, 'frame')
  if (frame?.kind !== 'frame') return
  assert.match(frame.srcDoc, /function __dshScheduleSize\(force\)\{if\(__dshSizeFrame\)return;__dshSizeFrame=requestAnimationFrame\(function\(\)\{__dshReportSize\(force===true\)\}\)\}/u)
  assert.match(frame.srcDoc, /setTimeout\(function\(\)\{__dshScheduleSize\(true\)\},250\);setTimeout\(function\(\)\{__dshScheduleSize\(true\)\},2000\)/u)
  assert.match(frame.srcDoc, /addEventListener\('load',function\(\)\{__dshScheduleSize\(true\)\}\)/u)
})
