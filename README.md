...
一轮中的工作按用途分开：搜索等写作前工具会把结果交还角色，并继续使用当前角色与预设完成正文；正文后生成的图片只进入精简的呈现交接，不会再次注入整套酒馆写作提示；MVU 状态在正文结束后由独立后台请求结算。玩家明确要求保存长期记忆时，角色可以在可见回复末尾调用 `remember`，保存成功后直接结束本轮，不再为工具回执重写一次正文。自动剧情总结、大纲维护与通用多 Agent 调度尚未启用，后续需要根据真实游玩反馈单独设计。
