...
export async function cleanGeneratedArtifacts(root: string, dryRun = false): Promise<readonly string[]> {
  const resolvedRoot = resolve(root)
  const entries = await readdir(resolvedRoot, { withFileTypes: true })
  const targets = entries.map(entry => entry.name).filter(isGeneratedArtifactName).sort()
  for (const name of targets) {
    ...
    if (!dryRun) await rm(target, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 })
  }
  return targets
}
...
if (process.argv[1] !== undefined && resolve(process.argv[1]) === resolve(scriptPath)) {
  ...
  if (args.some(arg => arg !== '--dry-run')) {
    throw new Error('Usage: pnpm clean:generated [-- --dry-run]')
  }
  const dryRun = args.includes('--dry-run')
  const root = resolve(fileURLToPath(new URL('..', import.meta.url)))
  const targets = await cleanGeneratedArtifacts(root, dryRun)
  if (targets.length === 0) {
